...
def post_slack_summary(stats, today):
    ...
    if total == 0:
        ...
        lines.append(f"*{stats['updates']} updated · {stats['creates']} created · 0 deleted*")
...
def run_sync(dry_run=False, lookback_days=14, lookahead_days=120):
    ...
    for entry in plan_cat2:
        ...
        if key in seen_cat2:
            continue
        seen_cat2.add(key)
        summary = catalog_summary(entry["identifier"])
        desc    = catalog_description()
        if d in catalog_events_by_date:
            print(f"  {d}  {summary!r} -- already exists, skip.")
        else:
            result = retry(create_event, cal_svc, summary, d, desc, dry_run)
            n_creates += 1
            n_cat2_creates += 1
            create_log.append({"date": d, "titles": [summary]})
            if result:
                affected_links.append(result.get("htmlLink", ""))

    stats = {"updates": n_updates, "creates": n_creates, "deletes": 0,
             "update_log": update_log, "create_log": create_log}
